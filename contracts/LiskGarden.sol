...
contract LiskGarden {
    ...
    // TODO 4.3: Event Frvested(uint256 indexed plantId, address indexed owner, uint256 reward)
    ...
    function plantSeed() external payable returns (uint256) {
        ...
        require(msg.value >= PLANT_PRICE, "Insufficient funds to plant seed");
        plantCounter++;
        myPlant = Plant({
            id: plantCounter,
            owner: msg.sender,
        ...
        userPlants[owner].push(plantCounter);
    ...
    function calculateWaterLevel(uint256 plantId) public view returns (uint256) {
        ...
        uint256 depletionIntervals = timeSinceWatered / WATER_DEPLETION_INTERVAL;
    ...
    function waterPlant(uint256 plantId) external {
        // TODO: Implement
        Plant storage plant = plants[plantId];
        require(plant.exists, "Tanaman Mati");
    ...
    function harvestPlant(uint256 plantId) external {
        ...
        require(!plant.isDead, "Tanaman Mati");
        updatePlantStage(plantId);
        require(plant.stage == GrowthStage.BLOOMING, "Stage harus blooming");
        plant.exists = false;
        uint256 reward = HARVEST_REWARD;
        emit PlantHarvested(plantId, owner, reward);

        (bool success, ) = msg.sender.call{value: 0.003 ether}("");
